...
<div style='margin:20px 40px 0 0;'>
    <h4>TEST RESULTS Mailed to Participants</h4>

    <p>Use <b>The Exact CSV</b> exported from the "results - not yet sent" reports.  <br>Upload back here to check them off as sent and they will no longer appear in the reports</p>
    ...
    <section id="pending_invites">
        ...
        <div class='qrscan'>
            ...
            <label for='upload_csv'></label><input type='file' name='upload_csv' id='upload_csv' placeholder="Test Results Sent CSV"/>
            </form>
            ...
            function ajaxlikeFormUpload(el){
                ...
                if($('iframe[name=iframeTarget]').length < 1){
                    ...
                    $(iframe).on('load', function(e) {
                        ...
                        if(success){
                            $("#result_msg").addClass("loaded").html("Success " + success + " of <b>" + total_rows + "</b> records successfully updated");
                        }else{
                            $("#result_msg").addClass("failed").html("Error : records not updated" + errors);
                        }
                    });
                }
